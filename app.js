const {
  ...
  Tray,
  nativeImage
} = require("electron"); // http://electron.atom.io/docs/api
...
app.once("ready", () => {
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          click: () => {
            window.webContents.session.getCacheSize(size =>
              dialog.showMessageBox({
                type: "info",
                buttons: ["OK"],
                message: `Cache size is: ${size} bytes.`
              })
            );
          }
        }
      ...
      submenu: [
        {
          ...
          click: () => {
            require("electron").shell.openExternal(
              "https://github.com/alexdevero/khan-academy-desktop-app"
            );
          }
        },
        {
          label: "Author",
          click: () => {
            require("electron").shell.openExternal("https://alexdevero.com");
          }
        }
      ]
    }
  ...
  // Create tray icon
  natImage = nativeImage.createFromPath(trayIcon);
  appIcon = new Tray(natImage);
